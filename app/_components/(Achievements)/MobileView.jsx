    ...
    <div className="relative grid grid-cols-[1px_250px] gap-6">
      ...
      <div className="my-8">
        <Container className={'max-w-[250px] p-4'}>
          ...
          <img src={ctx.eventImage} className="my-5 max-w-full rounded-xl" alt="Missing Alt!" />
          <div className="flex flex-row gap-3">
            <Container className={'w-full rounded-md p-1 text-center text-sm'}>{ctx.location}</Container>
            <Container className={'w-full rounded-md p-1 text-center text-sm'}>{new Date(ctx.date)}</Container>
          </div>
        </Container>
      </div>
    </div>
